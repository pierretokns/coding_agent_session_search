...
| `CASS_SEMANTIC_EMBED_WORKERS` | 1 | Native MiniLM workers for large semantic batches. Set `2` to use two independently loaded workers; this is bounded for predictable RSS and preserves vector order. |
| `CASS_SEMANTIC_MAX_RAW_MESSAGE_BYTES` | 16777216 | Reject one raw message larger than 16 MiB before transferring its body into a Rust semantic buffer. Set `0` only when unbounded input is intentional. |
